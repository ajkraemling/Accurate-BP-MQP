...
void BPMonitor::update(const BPMeasurement& m)
{
    ...
    // MAP detector only needs pressure and timestamp
    mapDetector.addSample(m.pressure, m.timestamp);
    ...
    switch (state)
    {
        case IDLE:
            if (m.pressure > 30)
            {
                ...
                maxPressure = m.pressure;  // Reset max pressure at start
            }
            break;
        ...
        case INFLATING:
            ...
            // Transition to MEASURING when pressure starts decreasing significantly
            // Wait for higher pressure and larger drop to ensure we're truly deflating
            if (maxPressure > 120 && m.pressure < maxPressure - 10)
            {
                state = MEASURING;
                calculateBaselineHeartRate(m.timestamp);
                
                // Reset and configure all detectors for measurement phase
                for (int i = 0; i < detectorCount; i++)
                {
                    detectors[i]->reset();  // Clear old baseline data from inflation
