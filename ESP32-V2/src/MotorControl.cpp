...
void MotorControl::startInflation(int speed) {
    if (inflating) return;
...
void MotorControl::stopInflation() {
    if (!inflating) return;
    setMotorSpeed(0);
    inflating = false;
}

void MotorControl::startDeflation(int rate) {
    if (deflating) return;
...
void MotorControl::stopDeflation() {
    if (!deflating) return;
...
void MotorControl::setMotorDirection(bool forward) {
    digitalWrite(MOTOR_IN1_PIN, forward ? HIGH : LOW);
    digitalWrite(MOTOR_IN2_PIN, forward ? LOW : HIGH);
}

void MotorControl::setSolenoidOpening(int opening) {
