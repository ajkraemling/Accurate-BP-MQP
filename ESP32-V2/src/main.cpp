...
const int THRESHOLD_UPDATE_INTERVAL = 5000; // Update thresholds every 5 seconds
 ...
 * Gradually adjusts min/max signal values and recalculates thresholds
 *
 * Purpose:
 *   Adapts to changing conditions (loosening grip, movement, etc.)
 *   without requiring full recalibration
 *
 * Method:
 *   - Runs every THRESHOLD_UPDATE_INTERVAL (5 seconds)
 *   - Uses exponential moving average (90% old, 10% new) for smooth updates
 *   - Only updates if current signal is near the extremes
 *   - Recalculates upper/lower thresholds based on new range
 *
 * This prevents the system from becoming "stuck" with outdated thresholds
...
void updateAdaptiveThresholds()
{
  ...
  if (currentTime - lastThresholdUpdate > THRESHOLD_UPDATE_INTERVAL)
  {
    bool thresholdsChanged = false;

    // Gradually update max if we're seeing high signals
    if (Signal > maxSignal - 100)
    {
      int oldMax = maxSignal;
      maxSignal = maxSignal * 0.9 + Signal * 0.1; // 90% old, 10% new
      if (abs(oldMax - maxSignal) > 10)
      {
        thresholdsChanged = true;
      }
    }

    // Gradually update min if we're seeing low signals (but not zero/noise)
    if (Signal < minSignal + 100 && Signal > 50)
    {
      int oldMin = minSignal;
      minSignal = minSignal * 0.9 + Signal * 0.1;
      if (abs(oldMin - minSignal) > 10)
      {
        thresholdsChanged = true;
      }
    }

    // Recalculate thresholds if values changed significantly
    if (thresholdsChanged)
    {
      int range = maxSignal - minSignal;
      upperThreshold = minSignal + (range * 0.46);
      lowerThreshold = minSignal + (range * 0.40);

      if (DEBUG_MODE)
      {
        Serial.println("\n[Thresholds adapted: Upper=" + String(upperThreshold) +
                       " Lower=" + String(lowerThreshold) + " Range=" + String(range) + "]");
      }
    }
  ...
  if (DEBUG_MODE)
  {
    ...
    if (currentTime - lastDisplayUpdate >= displayInterval)
    {
      ...
      while (signalStr.length() < 4)
        signalStr = " " + signalStr; // Adds spaces for consistent padding

      int pressureInt = round(pressure_mmHg);
      String pressureStr = String(pressureInt);
      while (pressureStr.length() < 4)
        pressureStr = " " + pressureStr; // Adds spaces for consistent padding
